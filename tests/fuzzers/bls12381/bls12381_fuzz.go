...
func FuzzCrossPairing(data []byte) int {
	...
	if !(bytes.Equal(cResult.Marshal(), bls12381.NewGT().ToBytes(kResult))) {
		panic("pairing mismatch gnark / gtwb ")
	}
...
func FuzzCrossG1Add(data []byte) int {
	...
	if !(bytes.Equal(cp.Marshal(), g1.ToBytes(&kp))) {
		panic("G1 point addition mismatch gnark / gtwb ")
	}
...
func FuzzCrossG2Add(data []byte) int {
	...
	if !(bytes.Equal(cp.Marshal(), g2.ToBytes(&kp))) {
		panic("G2 point addition mismatch gnark / gtwb ")
	}
...
func FuzzCrossG1MultiExp(data []byte) int {
	...
	for i := 0; i < 17; i++ {
		// note that gtwb/crypto/bls12381 works only with scalars <= 32bytes
	...
	if _, err := g1.MultiExp(&kp, gethPoints, gethScalars); err != nil {
		panic(fmt.Sprintf("G1 multi exponentiation errored (gtwb): %v", err))
	}
	// note that gtwb/crypto/bls12381.MultiExp mutates the scalars slice (and sets all the scalars to zero)
	...
	// compare result
	if !(bytes.Equal(cp.Marshal(), g1.ToBytes(&kp))) {
		panic("G1 multi exponentiation mismatch gnark / gtwb ")
	}
...
func getG1Points(input io.Reader) (*bls12381.PointG1, *gnark.G1Affine, error) {
	...
	// marshal gnark point -> gtwb point
	g1 := bls12381.NewG1()
	kp, err := g1.FromBytes(cpBytes)
	if err != nil {
		panic(fmt.Sprintf("Could not marshal gnark.G1 -> gtwb.G1: %v", err))
	}
	if !bytes.Equal(g1.ToBytes(kp), cpBytes) {
		panic("bytes(gnark.G1) != bytes(gtwb.G1)")
	}
...
func getG2Points(input io.Reader) (*bls12381.PointG2, *gnark.G2Affine, error) {
	...
	if err != nil {
		panic(fmt.Sprintf("Could not marshal gnark.G2 -> gtwb.G2: %v", err))
	}
	if !bytes.Equal(g2.ToBytes(kp), cpBytes) {
		panic("bytes(gnark.G2) != bytes(gtwb.G2)")
	}
