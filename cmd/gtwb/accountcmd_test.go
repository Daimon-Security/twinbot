...
func TestAccountListEmpty(t *testing.T) {
	gtwb := runGeth(t, "account", "list")
	gtwb.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gtwb := runGeth(t, "account", "list", "--datadir", datadir)
	defer gtwb.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gtwb := runGeth(t, "account", "new", "--lightkdf")
	defer gtwb.ExpectExit()
	gtwb.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gtwb := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer gtwb.ExpectExit()
	gtwb.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gtwb := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gtwb.ExpectExit()
	gtwb.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gtwb.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gtwb := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gtwb.StderrText(), m) {
...
// https://github.com/Daimon-Security/twinbot/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gtwb.SetTemplateFunc("keypath", func(file string) string {
