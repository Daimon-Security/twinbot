...
import (
	...
	"github.com/Daimon-Security/twinbot/accounts/keystore"
	"github.com/Daimon-Security/twinbot/common"
	"github.com/Daimon-Security/twinbot/common/fdlimit"
	"github.com/Daimon-Security/twinbot/consensus"
	"github.com/Daimon-Security/twinbot/consensus/clique"
	"github.com/Daimon-Security/twinbot/consensus/ethash"
	...
	"github.com/Daimon-Security/twinbot/crypto"
	"github.com/Daimon-Security/twinbot/eth"
	...
	"github.com/Daimon-Security/twinbot/eth/tracers"
	"github.com/Daimon-Security/twinbot/ethdb"
	"github.com/Daimon-Security/twinbot/ethstats"
	"github.com/Daimon-Security/twinbot/graphql"
	"github.com/Daimon-Security/twinbot/internal/ethapi"
	"github.com/Daimon-Security/twinbot/internal/flags"
	"github.com/Daimon-Security/twinbot/les"
	"github.com/Daimon-Security/twinbot/log"
	"github.com/Daimon-Security/twinbot/metrics"
	"github.com/Daimon-Security/twinbot/metrics/exp"
	"github.com/Daimon-Security/twinbot/metrics/influxdb"
	...
	"github.com/Daimon-Security/twinbot/p2p/nat"
	"github.com/Daimon-Security/twinbot/p2p/netutil"
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gtwb account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Twinbot client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Twinbot) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Twinbot service: %v", err)
		}
...
// RegisterEthStatsService configures the Twinbot Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Twinbot Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gtwb.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.GlobalString(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gtwb.", tagsMap)
		}
...
// new format.
//
// e.g. gtwb account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gtwb --keystore /tmp/mykeystore --lightkdf account new
//
