...
Use `devp2p enrdump <base64>` to verify and display an Twinbot Node Record.
...
1. initialize the gtwb node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run gtwb with the following flags:
```
gtwb --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the gtwb node:
 ```
...
To run the eth66 protocol test suite, initialize a gtwb node as described above and run the following command,
replacing `<enode>` with the enode of the gtwb node:
...
[dns-tutorial]: https://gtwbtwinbotchain.org/docs/developers/dns-discovery-setup
