...
func (g *gethrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.gtwb.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gethrpc) addPeer(peer *gethrpc) {
	g.gtwb.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gtwb.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gtwb.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gtwb.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gtwb.Error("timeout adding peer after", dur)
	}
}
...
func (g *gethrpc) waitSynced() {
	...
	if ok && !syncing {
		g.gtwb.Logf("%v already synced", g.name)
		return
	}
	...
	if err != nil {
		g.gtwb.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.gtwb.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.gtwb.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.gtwb.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.gtwb.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startGethWithIpc(t *testing.T, name string, args ...string) *gethrpc {
	ipcName := fmt.Sprintf("gtwb-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	ipcpath := ipcEndpoint(ipcName, g.gtwb.Datadir)
	// We can't know exactly how long gtwb will take to start, so we try 10
...
func initGeth(t *testing.T) string {
	...
	t.Logf("Initializing gtwb: %v ", args)
	g := runGeth(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
