...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light Twinbot peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), forkID, h.forkFilter, h.server); err != nil {
		p.Log().Debug("Light Twinbot handshake failed", "err", err)
		return err
	}
	...
	for {
		...
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light Twinbot message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light Twinbot message arrived", "code", msg.Code, "bytes", msg.Size)
