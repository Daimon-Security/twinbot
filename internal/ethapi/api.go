...
// PublicEthereumAPI provides an API to access Twinbot related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicEthereumAPI struct {
	b Backend
}

// NewPublicEthereumAPI creates a new Twinbot protocol API.
...
// Sign calculates an Twinbot ECDSA signature for:
...
//
// https://github.com/Daimon-Security/twinbot/wiki/Management-APIs#personal_sign
...
//
// https://github.com/Daimon-Security/twinbot/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Twinbot signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gtwb.
...
// PublicBlockChainAPI provides an API to access the Twinbot blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Twinbot blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current twinbot chain config.
...
// PublicDebugAPI is the collection of Twinbot APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Twinbot APIs exposed over the private
// debugging endpoint.
...
// Version returns the current twinbot protocol version.
