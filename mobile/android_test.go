...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.twinbot", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gtwb-android-")
	if err != nil {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/Daimon-Security/twinbot/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gtwb.aar"), "gtwb.aar")
