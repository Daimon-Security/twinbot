...
import (
	...
	"github.com/Daimon-Security/twinbot/crypto/signify"
	"github.com/Daimon-Security/twinbot/internal/build"
	"github.com/Daimon-Security/twinbot/params"
	"github.com/cespare/cp"
)

var (
	// Files that end up in the gtwb*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gtwb"),
	}

	// Files that end up in the gtwb-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Twinbot contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Twinbot bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (Twinbot Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "gtwb",
			Description: "Twinbot CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "Twinbot private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Twinbot account management tool.",
		},
	}
...
func doArchive(cmdline []string) {
	...
	var (
		...
		gtwb     = "gtwb-" + basegeth + ext
		alltools = "gtwb-alltools-" + basegeth + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gtwb, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gtwb, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gtwb-release.pub"
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gtwb-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gtwb-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Twinbot Builds <fjl@ethereum.org>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gtwb.exe" {
			gethTool = file
		} else {
	...
	// first section contains the gtwb binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gtwb.nsi", filepath.Join(*workdir, "gtwb.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("gtwb-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gtwb.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/Daimon-Security/twinbot/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gtwb.aar", filepath.Join(GOBIN, "gtwb.aar"))
		os.Rename("gtwb-sources.jar", filepath.Join(GOBIN, "gtwb-sources.jar"))
		return
	}
	...
	archive := "gtwb-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("gtwb.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/Daimon-Security/twinbot/mobile")
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "gtwb-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
