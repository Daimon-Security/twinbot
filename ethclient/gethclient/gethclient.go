...
// Package gethclient provides an RPC client for gtwb-specific APIs.
package gethclient
...
// Client is a wrapper around rpc.Client that implements gtwb-specific functionality.
//
// If you want to use the standardized Twinbot RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
// GCStats retrieves the current garbage collection stats from a gtwb node.
...
// MemStats retrieves the current memory stats from a gtwb node.
...
// GetNodeInfo retrieves the node info of a gtwb node.
